    ...
    const int32_t *data, uint32_t data_size)
{
    ...
    /* WORKAROUND v2 (2026-07-25, mixed_chains/fan8 Acrobat blank-tree bug --
       ...
       when a point_array value's bits END exactly at a byte boundary
       (bit position mod 8 == 7) within the decompressed tessellation
       bitstream. Confirmed via real-Acrobat testing across bit-lengths
       {9,10,12,13,14,22}: this "ends at a byte boundary" condition
       explains most (not all -- see the investigation doc for the known
       residual gap) of the specific (bit-length, phase) combinations
       found to fail, generalizing the original narrower "22 bits at
       phase 1" finding (which is the one instance where bit-length and
       phase were both pinned down exhaustively; end-of-field 22+1=23=7
       mod 8, matching this broader rule as a special case).

       Unlike v1, this condition is common enough (~1-in-8 entries by
       chance) that a rewind-and-retry loop would be too slow on large
       real arrays (each retry re-scanning from the start is fine for a
       handful of activations, not for potentially thousands). Instead:
       predict the bit-lengths table's position ASSUMING an uncompressed
       encoding (deterministic -- 1 flag bit + 32-bit size + 8 bits/entry,
       independent of the VALUES, so this prediction can't be invalidated
       by anything the pass below does) and do a SINGLE forward pass,
       self-correcting: if an entry's own end position would land at
       phase 7, bump ITS OWN bit-length by 1 (always lossless -- the
       reader just reads however many bits are declared). Since this only
       ever touches the CURRENT entry, never an earlier one, there is no
       rewind and no risk of re-breaking an already-processed entry --
       O(n) total, single pass. If nothing needed fixing, the normal
       (possibly Huffman-compressed) table encoding is used unchanged. */
    {
        /* prc_bitwrite_uint32 is a VARIABLE-length encoding (one
           continuation bit + one byte per significant byte of the value,
           then a terminating 0 bit -- see its own definition,
           prc_write_bit.c ~line 151), not a fixed 32 bits. The
           array_size field's actual bit cost therefore depends on
           data_size's own magnitude (9 bits per significant byte + 1,
           i.e. 10 bits for data_size <= 255, 19 for <= 65535, etc.) and
           must be computed the same way here, or this whole prediction
           silently drifts from the real on-disk position -- exactly the
           bug this comment replaces (an earlier version hard-coded 32
           bits here, correct only when data_size happened to need all 4
           bytes, and was wrong -- by exactly the saved bits -- for every
           smaller data_size, corrupting every phase calculation after
           it). */
        uint32_t array_size_nbytes = 0;
        uint32_t tmp = data_size;
        uint64_t pos;
        uint32_t fix_count = 0;

        while (tmp != 0) { array_size_nbytes++; tmp >>= 8; }

        pos = (uint64_t)state->byte_pos * 8 + state->bit_fill
            + 1 /* is_compressed flag bit */
            + (uint64_t)array_size_nbytes * 9 + 1 /* variable-length array_size field */
            + (uint64_t)data_size * 8; /* uncompressed table payload, 1 byte/entry */

        for (k = 0; k < data_size; k++)
        {
            if ((pos + bit_lengths[k]) % 8 == 7)
            {
                bit_lengths[k]++;
                fix_count++;
            }
        ...
        /* DIAGNOSTIC (2026-07-25, PRC_DIAG_PHASE1_GUARD): reports whether
           this guard activated for this array and how many entries it
           touched -- opt-in via getenv, zero behavior change when unset. */
        if (fix_count > 0 && getenv("PRC_DIAG_PHASE1_GUARD") != NULL)
            fprintf(stderr, "PRC_DIAG_PHASE1_GUARD: activated, data_size=%u fix_count=%u\n",
                data_size, fix_count);

        if (fix_count > 0)
        {
            /* Forced uncompressed table write (mirrors
               prc_bitwrite_character_array's own uncompressed branch,
               ~line 1472 below), matching the size just predicted above
               exactly -- deterministic, independent of the bit_lengths[]
               VALUES (only their COUNT, data_size, which is unchanged). */
        ...
        {
            /* matches: bit_lengths = prc_bitread_character_array(ctx,
               state, &size, 6, true, 0); (prc_bit.c 1435) -- the
               bit-length table itself, written (and possibly
               Huffman-compressed) before any of the values it
               describes. */
